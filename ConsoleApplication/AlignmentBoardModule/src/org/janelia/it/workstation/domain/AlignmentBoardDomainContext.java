package org.janelia.it.workstation.domain;
...
public class AlignmentBoardDomainContext {

    private static final Logger log = LoggerFactory.getLogger(AlignmentBoardDomainContext.class);

    private AlignmentContext context;
    private AlignmentBoard alignmentBoard;
    
    public AlignmentBoardDomainContext(AlignmentBoard alignmentBoard) {
        String as = alignmentBoard.getAlignmentSpace();      // getInternalEntity().getValueByAttributeName(EntityConstants.ATTRIBUTE_ALIGNMENT_SPACE);
        String ores = alignmentBoard.getOpticalResolution(); //getInternalEntity().getValueByAttributeName(EntityConstants.ATTRIBUTE_OPTICAL_RESOLUTION);
        String pres = alignmentBoard.getImageSize();   //getInternalEntity().getValueByAttributeName(EntityConstants.ATTRIBUTE_PIXEL_RESOLUTION);
...
/*    
        if (domainObject instanceof Sample) {
            addNewAlignedEntity((Sample)domainObject);
        }
        else if (domainObject instanceof NeuronFragment) {
            Sample sample = new DomainHelper().getSampleForNeuron((NeuronFragment)domainObject);
            NeuronSeparation ns = getPipelineAncestor(sample);
            AlignmentResult ar = DomainMgr.getAlignmentResultForSeparation(ns);
            if (ar == null) {
                JOptionPane.showMessageDialog(SessionMgr.getMainFrame(), "Neuron is not aligned", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            if (! isCompatibleAlignmentSpace(domainObject, ns, ar)) {
                return;
            }
            
            for (NeuronFragment nf: sample.getObjectiveSample("20x").getPipelineRuns().get(0).getLatestAlignmentResult().get) {
...
//        }
            ...
            Sample sample = new Sample();//(Sample) EntityWrapperFactory.wrap(new RootedEntity(sampleEntity));
