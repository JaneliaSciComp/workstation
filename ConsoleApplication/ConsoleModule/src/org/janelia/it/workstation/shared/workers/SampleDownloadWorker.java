...
public class SampleDownloadWorker extends SimpleWorker {
    ...
    protected void doStuff() throws Exception {
        ...
        if (EntityConstants.TYPE_SAMPLE.equals(selectedEntity.getEntityTypeName())) {
            ...
            sampleEntity = modelMgr.getAncestorWithType(selectedEntity, EntityConstants.TYPE_SAMPLE);
        }

        if (sampleEntity == null) {
            sampleEntity = selectedEntity;
        }
        sampleEntityName = sampleEntity.getName();
        sampleTargetDir = new File(rootDownloadDir, sampleEntityName);
        sourceFilePath = EntityUtils.getDefault3dImageFilePath(selectedEntity);

        final Entity targetLoaded = modelMgr.loadLazyEntity(selectedEntity, false);
        final Entity default3dImage = targetLoaded.getChildByAttributeName(EntityConstants.ATTRIBUTE_DEFAULT_3D_IMAGE);

        String idStr;
        if (default3dImage == null) {
            idStr = "";
        } else {
            idStr = "_ID" + default3dImage.getId();
        }
    ...
    private void convertOnServer(final String localFilePrefix) throws Exception {
        ...
        if (splitChannels) {
            ...
            task = modelMgr.submitJob("ConsoleSplitChannels", "Split Channels", taskParameters);
        }
        else {
            HashSet<TaskParameter> taskParameters = new HashSet<>();
            taskParameters.add(new TaskParameter("filepath", sourceFilePath, null));
            taskParameters.add(new TaskParameter("output extension", extension, null));
            task = modelMgr.submitJob("ConsoleConvertFile", "Convert File", taskParameters);
        }
