...
public class MaskSearchDialog extends ModalDialog {
    ...
    public MaskSearchDialog() {
        ...
        if (null!= SessionMgr.getSessionMgr().getModelProperty(PREF_MASK_SEARCH_SERVICE_FOLDER_NAME)&&
            ...
            folderField.setText(AccessManager.getUsername()+TOP_LEVEL_FOLDER_NAME);
...
//                browser.setPerspective(Perspective.TaskMonitoring);
    ...
    private void startMaskSearch(String path, String topLevelFolderName, String matrixValue, String queryChannel,
        ...
        try {
            ...
            searchRequest = StateMgr.getStateMgr().submitJob(process, task);
...
//                            ArrayList<Entity> tmpFolder = (ArrayList<Entity>) StateMgr.getStateMgr().getEntitiesByName(folderField.getText().trim());
//                            if (null!=tmpFolder && tmpFolder.size()>0 && null!=tmpFolder.get(0).getId()) {
//                                final Entity tmpFolderEntity = StateMgr.getStateMgr().getEntityById(tmpFolder.get(0).getId());
...
//                                    @Override
//                                    public Void call() throws Exception {
//                                        StateMgr.getStateMgr().getEntitySelectionModel().selectEntity(EntitySelectionModel.CATEGORY_OUTLINE,
...
//            StateMgr.getStateMgr().getLoaderThreadQueue().addQueue(searchWatcher);
//            waitForLoading(taskStatus);
    ...
    final class SearchWatcher extends TaskThreadBase {
        ...
        public void run() {
            ...
            try {
                Task task = StateMgr.getStateMgr().getTaskById(this.taskRequest.getTaskFilter().getTaskId());
                if (monitorState) {
                    if (task.isDone()) {
