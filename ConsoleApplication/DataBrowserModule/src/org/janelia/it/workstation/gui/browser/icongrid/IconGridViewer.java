...
//    protected Hud hud;
    ...
    protected List<T> pageImageObjects;
...
//    }
    ...
    private synchronized void goStartPage() {
        showImageObjects(0, null);
    }

    private synchronized void goEndPage() {
        showImageObjects(numPages - 1, null);
    }
...
//            ModelMgr.getModelMgr().getEntitySelectionModel().selectEntity(getSelectionCategory(), rootedEntity.getId(), false);
    ...
    public void showImageObjects(List<T> imageObjects) {
        if (imageObjects.isEmpty()) {
            imageObjects = new ArrayList<T>();
            imageObjects.add(getContextObject());
        }
        showImageObjects(imageObjects, null);
    }
    
    public void showImageObjects(List<T> imageObjects, final Callable<Void> success) {
        this.allImageObjects = imageObjects;
        this.numPages = (int) Math.ceil((double) allImageObjects.size() / (double) PAGE_SIZE);
        showImageObjects(0, success);
    }
    
    private synchronized void showImageObjects(final int pageNum, final Callable<Void> success) {
        
        this.currPage = pageNum;

        int i = pageNum * PAGE_SIZE;
        int j = i + PAGE_SIZE;
        if (j > allImageObjects.size()) {
            j = allImageObjects.size();
        }

        final List<T> pageObjects = allImageObjects.subList(i, j);
        ...
        log.info("Setting {} image objects",pageObjects.size());
        setImageObjects(pageObjects);
...
//    }
    ...
    public synchronized List<T> getPageImageObjects() {
        return pageImageObjects;
    }
