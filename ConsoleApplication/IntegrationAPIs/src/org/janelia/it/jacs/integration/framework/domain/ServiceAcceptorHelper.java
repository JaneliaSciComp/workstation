...
public class ServiceAcceptorHelper {
    ...
    public static Collection<DomainObjectHelper> findHelpers(DomainObject domainObject) {
        return findHandler(domainObject, DomainObjectHelper.class, DomainObjectHelper.DOMAIN_OBJECT_LOOKUP_PATH);
    }
    
    public static DomainObjectHelper findFirstHelper(DomainObject domainObject) {
        Collection<DomainObjectHelper> handlers = findHelpers(domainObject);
        if (handlers.isEmpty()) return null;
        return handlers.iterator().next();
    }
