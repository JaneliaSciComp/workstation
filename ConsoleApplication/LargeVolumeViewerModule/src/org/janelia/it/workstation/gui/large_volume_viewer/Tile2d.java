 ...
 *
 * A particular Tile2d may use a lower resolution TileTexture, in case its
 * corresponding full-resolution TileTexture is not yet available.
 *
 * There is a subtle distinction between a Tile2d and a TileTexture. A Tile2d
 * represents a square block of pixels on the screen viewport. A TileTexture
 * represents a square image loadable from the data store. There is usually a
 * one-to-one correspondence between Tile2d and TileTexture. But sometimes a
 * Tile2d might use a sub-region of a lower resolution TileTexture.
 *
 * @author brunsc
 *
 */
public class Tile2d implements GLActor {
    ...
    private int[] getWhdToXyz() {
        ...
        if (sliceAxis == CoordinateAxis.X) {
            whdToXyz = new int[]{2, 1, 0};
        } else if (sliceAxis == CoordinateAxis.Y) {
            whdToXyz = new int[]{0, 2, 1};
        }
        return whdToXyz;
    }
