 ...
 * Time: 3:30:30 PM
 */
class SampleAccessionSearchResultBuilder extends AccessionSearchResultBuilder {

    SampleAccessionSearchResultBuilder() {
    }
    ...
    protected SQLQuery createSearchQuery(Object[] accessionQueryParams, Session session) {
        ...
        String sql = "select " +
                ...
                "from " +
                "bio_sample sample " +
                "where upper(sample_acc) = :accession ";
        SQLQuery sqlQuery = session.createSQLQuery(sql);
        _logger.debug("Sample accession search sql: " + sql + " for " + searchedAcc);
