...
public class CustomTreeFind {
    ...
    public Node find() {
        ...
        hasRun = true;
        Node foundNode = find(dynamicTree.getRootNode());
        if (foundNode != null) {
            return foundNode;
        }
        return firstMatch;
    ...
    private Node find(Node currNode) {
        ...
        try {
            ...
            Set<Method> isInitMethods = ReflectionUtils.getAllMethods(Children.class,
                    ReflectionUtils.withModifier(Modifier.PROTECTED),
                    ReflectionUtils.withName("isInitialized"));
