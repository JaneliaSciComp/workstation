 ...
 * wish to work with default and user-defined preference objects. The object
 * definition maintains a "dirty" state and this class manages the lifecycle of
 * the object. The steps of operation are: subclass provides filenames, this
 * class reads default and user property files and organizes keys hierarchially
 * (user overriding default), the subclass defines the factory to create their
 * specific type of InfoObjects and the methods to work with those objects. This
 * class commits changes or rebuilds the collections as needed, and finally
 * writes out the user-defined properties upon system exit. As example, see the
 * GB ViewPrefMgr.java class.
 */
public abstract class PreferenceManager {
    ...
    /**
     * The Master collections of Default, User, and Dirty Info Objects These
    ...
    /**
     * Prep the source collections to be re-read from scratch. Build the User,
    ...
    public void setGroupPreferenceDirectory(String newDir) {
        if (newDir != null && !newDir.equals("")) {
            ...
            if (dirFiles != null) {
                ...
                // These four actions might need to go into a reset method or
                // something.
    ...
    /**
     * This method compiles the possible data sources for User or Default and
     * passes that information on to populate the Info Objects for the specific
     * Master collection.
     */
    private void buildInfoCollection(int mode) {
        ...
        try {
            if (mode == DEFAULT)
                targetFile = this.getClass().getResourceAsStream(filename);
            else if (mode == USER) {
     ...
     * out the file locations for default user, group, and the file filter to be
     * used.
     */
    public abstract void setFileParameters();

    /**
     * This method sifts through the Java property keys and puts them in
     * sub-groups by type. It then passes those Properties objects on to the
     * build methods. Remember this is for only one source, Default or Group or
     * User, at a time.
     */
    /**
     ...
     */
    protected abstract void populateInfoObjects(Properties allProperties, Map<String, InfoObject> targetMasterCollection, String sourceFile);

    /**
     * This method is supposed to hierarchially add info objects to the final
     * collections. Any deletion should be a cause for remerging; for example,
     * if a user defined info object is removed it could be replaced with a
     * group or default object.
     */
    /**
     * This method should be used by the subclasses to populate their own
     * collections from the items gained during property file load.
     */
    protected abstract void mergeIntoWorkingCollections(Map<String, InfoObject> targetMasterCollection);

    /**
     * This method is intended to be overridden so that the subclass can
     * determine how to notify the user that the default keys cannot be deleted.
     */
    public abstract void handleDefaultKeyOverrideRequest();

    /**
     * This method is intended to be overridden by the subclass. The subclass
     * will then send all of its proprietary collections to formatOutput()
     * one-by-one.
     */
    ...
    /**
     * This method should be overridden in order to determine what notification
     * the user receives from the error.
     */
    ...
    /**
     * This helper method looks through the properties handed to it and returns
     * a set of the keys unique by the first string token.
     */
     ...
     * Master collections; merging them into their usable state. Before calling:
     * subclasses clear out their specific InfoObject collections.
     */
     ...
     * happened to a Preference Info Object. Finer granularity will be added in
    ...
    /**
     * This method should be used when panels bulk modify Info Objects in the
     * Working collections or when one change is made that should be saved out
     * when the session exits. If the Working collection has dirty objects that
     * should be cancelled then commitPermanently should be false; true, and
     * they should be written to the Master collection.
     */
    /**
    ...
    /**
     * Creates a list of possible output files that match this filter. Perhaps
     * so a subclass can make a user select one to save changes to from a list.
     */
    public ArrayList<File> getAvailableOutputFiles() {
    ...
    /**
     * This highly-used method toggles a string from the unique-key state to the
     * readable string that the user sees. The conversion is necessary for keys
    ...
    /**
     * @todo This class has to be overridden in the subclasses in order for them
     *       to work yet this is not forced. Need an abstract class to make this
     *       work better.
     */
    /**
     * This is the private method that does all the real committing work. First,
     * Info Objects added to the deletedInfo collection attempt to be removed
     * from the Master collection that has influence in the hierarchy. Deletion
     * checks uncommitted Info's first. Then User->Group->Default. As it
     * proceeds, it keeps track of which sourcefile will need to be written back
     * to because of deletion. After this, it calls
     * handleDirtyOrUnknownInfoObjects() on the Working collections. Once done,
     * it resets the Working collections.
     */
    protected void commitChangesToSourceObjects() {
        // First remove the unwanteds. Only need to check top of hierarchy on
        // down.
        // If the object is found in the user map, bingo.
        for (String o : deletedInfos.keySet()) {
            InfoObject tmpObject = deletedInfos.get(o);
     ...
     * These have hierarchy precendence over all other infos of the same name
     * and will be written out at session exit.
     */
     ...
     * Sometimes used at systemWillExit and sometimes used to start the change
     * of user pref files.
     */
     ...
     * file = destination file, asks the Info Object for its property output and
     * writes the key/value pairs to the destination file.
     */
            ...
            for (Object o1 : workingCollection.keySet()) {
                ...
                if (tmpObject.getSourceFile().equals(destinationFile)) {
                    ...
                    for (Object o : tmpProperties.keySet()) {
                        ...
                        if (DEBUG)
                            System.out.println("Writing out: " + tmpKey + " " + tmpValue);
    ...
    /**
     * This area of the code is reserved for inner classes that serve this
     * class.
     */

    public class MyStringComparator implements Comparator<Object> {
