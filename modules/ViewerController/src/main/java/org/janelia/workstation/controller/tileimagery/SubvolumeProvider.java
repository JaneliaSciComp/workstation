...
public class SubvolumeProvider {
    ...
    public SubvolumeProvider(SharedVolumeImage volumeImage, TileServer tileServer) {
     ...
     */
    public Subvolume getSubvolumeFor3D(Vec3 center, double micrometerVoxels, int[] extent, int zoomIndex, IndeterminateNoteProgressMonitor monitor) {
        ZoomLevel zoomLevel = new ZoomLevel(zoomIndex);
        return new Subvolume(center, volumeImage, micrometerVoxels, zoomLevel, extent, tileServer.getTextureCache(), monitor);
    }
