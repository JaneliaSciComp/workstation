package org.janelia.workstation.gui.static_view;
...
import org.janelia.workstation.gui.viewer3d.DirectionalAxis;
...
public abstract class AbstractVolumeBrick implements VolumeBrickI
{
	...
	public void init(GLAutoDrawable glDrawable) {
        ...
        if (bTexturesNeedUploaded) {            
            ...
            if (OpenGLUtils.reportError(gl, "AVB, after uploading all textures.")) {
                return;
            }
        }
		if (bUseShader) {
        ...
        if (bBuffersNeedUpload) {
            try {
                ...
                if (OpenGLUtils.reportError( gl, "building buffers" )) {
	...
	public void displayVolumeSlices(GL2 gl) {
        ...
        if (OpenGLUtils.reportError(gl, "Display Volume Slices, on entry.")) {
            return;
        }
        ...
        if (OpenGLUtils.reportError(gl, "Volume Brick, after draw.")) {
            return;
        }
