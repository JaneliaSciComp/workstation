...
public class AttributeManagerBufferUploader implements BufferUploader {
    ...
    private final MeshDrawActor.MeshDrawActorConfigurator configurator;
    
    public AttributeManagerBufferUploader( MeshDrawActor.MeshDrawActorConfigurator configurator ) {
    ...
    public void uploadBuffers(GL2GL3 gl) throws BufferStateException {
        ...
        if (OpenGLUtils.reportError(gl, "Gen Buffer Handles.")) {
        ...
        for (Long renderId : renderIdToBuffers.keySet()) {
            ...
            if (attribBuffer != null && attribBuffer.capacity() > 0) {
                long bufferBytes = (long) (attribBuffer.capacity() * (MeshDrawActor.BYTES_PER_FLOAT));
                combinedVtxSize += bufferBytes;

                IntBuffer inxBuf = buffersBean.getIndexBuffer();
                bufferBytes = inxBuf.capacity() * MeshDrawActor.BYTES_PER_INT;
        ...
        if (OpenGLUtils.reportError(gl, "Allocate Vertex Buffer")) {
            throw new BufferStateException();
        }
        ...
        if (OpenGLUtils.reportError(gl, "Allocate Index Buffer")) {
            throw new BufferStateException();
        }
        ...
        for (Long renderId : renderIdToBuffers.keySet()) {
            ...
            if (attribBuffer != null && attribBuffer.capacity() > 0) {
                long bufferBytes = (long) (attribBuffer.capacity() * (MeshDrawActor.BYTES_PER_FLOAT));
                gl.glBindBuffer(GL2GL3.GL_ARRAY_BUFFER, getVtxAttribBufferHandle());
                if (OpenGLUtils.reportError(gl, "Bind Attribs Buf")) {
                    throw new BufferStateException();
                }
                ...
                if (OpenGLUtils.reportError(gl, "Buffer Data")) {
                    throw new BufferStateException();
                }
                //OpenGLUtils.dumpFloatBuffer(attribBuffer);

                IntBuffer inxBuf = buffersBean.getIndexBuffer();
                bufferBytes = (long) (inxBuf.capacity() * MeshDrawActor.BYTES_PER_INT);

                gl.glBindBuffer(GL2GL3.GL_ELEMENT_ARRAY_BUFFER, getInxBufferHandle());
                if (OpenGLUtils.reportError(gl, "Bind Inx Buf")) {
                    throw new BufferStateException();
                }
                ...
                if (OpenGLUtils.reportError(gl, "Upload index buffer segment.")) {
                    throw new BufferStateException();
                }
