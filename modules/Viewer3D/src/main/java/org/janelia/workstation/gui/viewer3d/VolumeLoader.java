...
//                textureData.setExplicitVoxelComponentType(GL2.GL_UNSIGNED_BYTE);
    ...
    private VolumeDataAcceptor.TextureColorSpace resolveColorSpace(String baseName, String extension) {
        // Default to linear color space
        // But look for some exceptions we know about
        VolumeDataAcceptor.TextureColorSpace colorSpace = VolumeDataAcceptor.TextureColorSpace.COLOR_SPACE_LINEAR;
        if (baseName.startsWith(VolumeFileLoaderI.CONSOLIDATED_SIGNAL_FILE))
            colorSpace = VolumeDataAcceptor.TextureColorSpace.COLOR_SPACE_SRGB;
