...
public class MatrixManager {
    ...
    private static final double ASSUMED_FOCUS_DISTANCE = -BaseRenderer.DISTANCE_TO_SCREEN_IN_PIXELS * 1.5;
    ...
    private double glUnitsPerPixel() {
        return Math.abs(getFocusDistance()) / BaseRenderer.DISTANCE_TO_SCREEN_IN_PIXELS;
    }
    ...
    private void updateProjection(GL2GL3 gl, double near, double far) {
        ...
        double verticalApertureInDegrees = 180.0 / Math.PI * 2.0 * Math.abs(
                Math.atan2(getHeightInPixels() / 2.0, BaseRenderer.DISTANCE_TO_SCREEN_IN_PIXELS));
