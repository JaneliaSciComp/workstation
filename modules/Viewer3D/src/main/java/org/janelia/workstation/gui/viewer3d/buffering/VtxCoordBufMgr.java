...
public class VtxCoordBufMgr {
     ...
     * @return True if buffers enabled, false if it failed.
    ...
    private int[] enableBuffersOfType(GL2 gl, Buffer[] buffers, int type, int classSize ) {		
        ...
        if (OpenGLUtils.reportError(gl, "Generating GL buffers")) {
            rtnVal = null;
        }
        else {
            ...
            for (int i = 0; i < NUM_BUFFERS_PER_TYPE; i++) {
                ...
                if (OpenGLUtils.reportError(gl, "Binding buffer " + i)) {
                    rtnVal = null;
                }
                else {
                    ...
                    if (OpenGLUtils.reportError(gl, "Pushing buffer " + i)) {
                        rtnVal = null;
                    }
                }
